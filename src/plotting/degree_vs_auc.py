...
DEGREE_REGEX = re.compile(
    r"(?:regular_degree[^0-9]*|regular_)([0-9]+)",
    re.IGNORECASE,
)
...
def parse_degree_from_filename(path: str) -> int:
    ...
    m = DEGREE_REGEX.search(base)
    if not m:
        raise ValueError(f"Could not find regular degree in filename: {path}")
    return int(m.group(1))
...
def load_runs(files: list[str], fixed_round: int) -> list[dict]:
    ...
    for f in files:
        ...
        row = pick_fixed_or_last_row(
            per_round, fixed_round, metric_cols=("auc", "test_acc")
        )
        ...
        runs.append(
            {
                ...
                "auc": float(row["auc"]) if "auc" in row and pd.notna(row["auc"]) else np.nan,
                "test_acc": float(row["test_acc"])
                if "test_acc" in row and pd.notna(row["test_acc"])
                else np.nan,
...
def aggregate_over_seeds(runs_df: pd.DataFrame) -> pd.DataFrame:
    ...
    Nodes are already averaged within a run.
    """
...
def plot_auc_vs_degree_per_alpha(agg: pd.DataFrame, out_dir: str, fixed_round: int):
    ...
    for alpha in sorted(agg["alpha"].unique()):
        df = (
            agg[agg["alpha"] == alpha]
            .dropna(subset=["degree", "mean_auc", "std_auc"])
            .sort_values("degree")
        )
        if df.empty:
        ...
        plt.xlabel("Topology degree (regular)")
        plt.ylabel("Mean AUC")
...
def main():
    ...
    runs_df.to_csv(
        os.path.join(args.output_dir, "runs_picked_metrics.csv"), index=False
    )

    agg = aggregate_over_seeds(runs_df)
    agg.to_csv(
        os.path.join(args.output_dir, "auc_acc_by_alpha_degree.csv"), index=False
    )

    plot_auc_vs_degree_per_alpha(
        agg, out_dir=args.output_dir, fixed_round=args.fixed_round
    )

    if not args.no_combined_plot:
        plot_auc_vs_degree_all_alphas(
            agg, out_dir=args.output_dir, fixed_round=args.fixed_round
        )
