...
def get_args():
    ...
    parser.add_argument('--dp', dest='enable_dp', action='store_true', help='Enable differential privacy (DP-SGD) to protect training data.')
    ...
    parser.add_argument("--lira-known-member-perc", type= float, default=0.05, help="Percentage of victim node training data that is known as member to the attacker to train shadow model.")
    parser.add_argument("--lira-known-nonmember-perc", type=float, default=0.5, help="Non-member fraction used for shadow training. A value of 0.1 means the attacker knows non-member samples equal to 10%% of the victim's training set size (drawn from test distribution).")
    parser.add_argument("--lira-num-shadow-models", type=int, default=16, help="Amount of shadow models the attacker trains.")
    ...
    parser.add_argument("--er-p", type=float, default=0.2, help="Edge probability p for Erdos-Renyi topology.")
    parser.add_argument("--sbm-num-blocks", type=int, default=2, help="Number of blocks (communities) for SBM topology.")
    parser.add_argument("--sbm-p-in", type=float, default=0.5, help="Intra-block edge probability for SBM topology.")
    parser.add_argument("--sbm-p-out", type=float, default=0.05, help="Inter-block edge probability for SBM topology.")
    parser.add_argument("--ws-k", type=int, default=4, help="Each node is joined with its k nearest neighbors in Watts-Strogatz topology.")
    parser.add_argument("--ws-p", type=float, default=0.1, help="Rewiring probability p for Watts-Strogatz topology.")
    parser.add_argument("--topology-seed", type=int, default=56, help="Controls randomness in topology creation.")
